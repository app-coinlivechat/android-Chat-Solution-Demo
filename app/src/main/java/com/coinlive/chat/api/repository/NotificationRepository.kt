...
object NotificationRepository {
    ...
    fun setNotification(auth:String, coinId:String, notiType:String) : Boolean{
        ...
        if(!response.isSuccess() && response.d == null){
            throw RequestFailException("NotificationRepository.setNotification fail. please check auth or coinId or " +
                    "notiType",
    ...
    fun deleteNotification(auth:String, coinId:String, notiType:String) : Boolean{
        ...
        if(!response.isSuccess() && response.d == null){
            throw RequestFailException("NotificationRepository.deleteNotification fail. please check auth or coinId or " +
                    "notiType",
                response.code,response.msg)
        }
        return true
    ...
    fun getNotificationType(auth:String, coinId:String) : List<NotificationType>{
        ...
        if(!response.isSuccess() && response.d == null){
            throw RequestFailException("NotificationRepository.getNotificationType fail. please check auth or coinId ",
                response.code,response.msg)
        }
