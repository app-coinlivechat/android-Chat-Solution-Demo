...
object UploadRepository {
    ...
    fun uploadImage(auth:String, file:List<Int>) : String{
        ...
        if(!response.isSuccess() && response.d == null){
            throw RequestFailException("UploadRepository.uploadImage fail. please check auth or file ",
    ...
    fun deleteImage(auth:String, url:String) : String{
        ...
        if(!response.isSuccess() && response.d == null){
            throw RequestFailException("UploadRepository.deleteImage fail. please check auth or url ",
                response.code,response.msg)
        }
        return response.d!!.url
    }
    ...
    fun uploadProfile(auth:String, file:List<Int>,isNft:Boolean) : String{
        ...
        if(!response.isSuccess() && response.d == null){
            throw RequestFailException("UploadRepository.uploadProfile fail. please check auth or file",
                response.code,response.msg)
        }
        return response.d!!.url
    }
