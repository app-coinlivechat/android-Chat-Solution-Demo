...
class LoginFragmentViewModel : ViewModel() {
                ...
                viewModelScope.launch {
                    clApi.isAvailableNickName(nickName, customer!!.id, object : ResponseCallback<Boolean> {
                        override fun onSuccess(value: Boolean) {
                            viewModelScope.launch {
                                clApi.getCustomToken(DemoApplication.password,
                                    customer!!.name,
                                    uuid,
                                    object : ResponseCallback<CustomerUserSignUp> {
                                        override fun onSuccess(value: CustomerUserSignUp) {
                                            viewModelScope.launch {
                                                ...
                                                clApi.signupCheck(firebaseUuid,
    ...
    fun logIn(customerName : String,uuid: String) = viewModelScope.launch {
        ...
        clApi.getCustomerInfo(customerName, object : ResponseCallback<Customer> {
            override fun onSuccess(value: Customer) {
                customer = value
                viewModelScope.launch {
                    clApi.getCustomToken(DemoApplication.password, customer!!.name, uuid, object :
    ...
    private suspend fun signUpCustomerUser(nickName: String, profileUrl: String?) = viewModelScope.launch {
        clApi.customerUserSignUp(CustomerUserSignUpBody(customer!!.id,
